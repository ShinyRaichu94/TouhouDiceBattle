	...
	if(PlayerControl == true)
	{
		...
		if(PlayerController == "Keys")
		{
			...
			if keyboard_check_released(ord("Z")){
				...
				var shootdir = shootx;
				var shootpower = (variableself_shootpower * -1)
				with(instance_create_layer(shootx, shooty, "Snowballs", SSO_Object_P2Snowball)){
					speed = shootpower;
					direction = shootdir;
	...
	else
	{
		...
		if (PlayerCPU = "Easy")
		{
			...
			if (CPUAimChangeTime <= 0){
				...
				CPUAimSetValue = random_range(-45,-20);
				if (global.SSO_Wind <= 0){CPUAimAddValue = random_range(-20,10);}
				else if (global.SSO_Wind > 0){CPUAimAddValue = random_range(-10,20);}
			}
			else {CPUAimChangeTime -= 1;}
		...
		else if (PlayerCPU = "Hard")
		{
			var PlayerCPUAim = 0;
			if (CPUAimChangeTime <= 0){
				CPUAimChangeTime = random_range(-40,-10);
				if (global.SSO_Wind >= 10){CPUAimSetValue = random_range(-45,-35);}
				else if (global.SSO_Wind < 10 && global.SSO_Wind >= 0){CPUAimSetValue = random_range(-35,-25);}
				else if (global.SSO_Wind < 0 && global.SSO_Wind <= -10){CPUAimSetValue = random_range(-25,-10);}
		...
		else if (PlayerCPU = "Lunatic")
		{
			var PlayerCPUAim = 0;
			if (CPUAimChangeTime <= 0){
				CPUAimChangeTime = random_range(-30,-10);
				if (global.SSO_Wind >= 10){CPUAimSetValue = random_range(-45,-35);}
				else if (global.SSO_Wind < 10 && global.SSO_Wind >= 0){CPUAimSetValue = random_range(-35,-25);}
				else if (global.SSO_Wind < 0 && global.SSO_Wind >= -10){CPUAimSetValue = random_range(-25,-10);}
				else {CPUAimSetValue = -10;}
