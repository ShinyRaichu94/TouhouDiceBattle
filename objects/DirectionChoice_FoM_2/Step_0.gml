...
if(PlayerControl == true){
	...
	if(PlayerController == "Keys"){
			...
			if (global.PlayerTurn == 1){
				if (DirectionSelect == 1){
					with(Object_BoardPlayer1){
						PathVariable = Path_Board_FoM_13;
						path_start(Path_Board_FoM_13, path_speed, path_action_stop, true);
						path_position = 0;
					}
				}
				else{
					with(Object_BoardPlayer1){
						PathVariable = Path_Board_FoM_4;
						path_start(Path_Board_FoM_4, path_speed, path_action_stop, true);
						path_position = 0;
					}
				}
			}
		...
		if (PlayerGamepadH >= 0.5){
			DirectionSelect = 1; // East
		}
		else if (PlayerGamepadV <= -0.5){
			DirectionSelect = 0; // North
		}
